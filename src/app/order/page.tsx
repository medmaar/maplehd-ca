...
function OrderContent() {
  ...
  const whatsappMsg = encodeURIComponent(
    `Hi MapleHD, I'd like to order the ${duration} plan for ${devices} device${Number(devices) > 1 ? "s" : ""} at $${price}.`
  );
  const emailSubject = encodeURIComponent(`MapleHD Order - ${duration} / ${devices} Device${Number(devices) > 1 ? "s" : ""}`);
  const emailBody = encodeURIComponent(
    `Hi,\n\nI'd like to purchase the following plan:\n\n- Duration: ${duration}\n- Devices: ${devices}\n- Price: $${price}\n\nPlease send me payment instructions.\n\nThank you!`
  );
  const telegramMsg = encodeURIComponent(
    `Hi MapleHD, I want to order the ${duration} plan for ${devices} device${Number(devices) > 1 ? "s" : ""} at $${price}.`
  );
    ...
    <main className="bg-gray-950 text-white min-h-screen py-16 px-4">
      <div className="max-w-xl mx-auto">
        ...
        <div className="space-y-4 mb-8">
          ...
          <a
            href={`mailto:help@maplehd.ca?subject=${emailSubject}&body=${emailBody}`}
